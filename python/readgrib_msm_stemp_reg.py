...
def plotmap(fcst_time, sta, lons_1d, lats_1d, lons, lats, rain, tmp, uwnd, vwnd, wspd, title, output_filename):
    #
    # MapRegion Classの初期化
    ...
    # 2度ごとに線をひく
    levels2 = range(math.floor(tmp.min()-math.fmod(tmp.min(),2)), math.ceil(tmp.max())+1,2)
    # 1度ごとに線をひく
    levels1 = range(math.floor(tmp.min()-math.fmod(tmp.min(),1)), math.ceil(tmp.max())+1,1)
    # 等温線をひく
    cmap = plt.get_cmap('seismic') # 色テーブルの選択
    if opt_c1:
        m.contour(lons, lats, tmp, levels=levels1, cmap=cmap, linestyles=':', linewidths=0.8)
...
if __name__ == '__main__':
    ...
    for fcst_time in np.arange(fcst_str, fcst_end+1, fcst_step):
        ...
        plotmap(fcst_time, sta, lons_1d, lats_1d, lons, lats, rain, tmp, uwnd, vwnd, wspd, \
