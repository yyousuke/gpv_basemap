...
def plotmap(fcst_time, sta, lons_1d, lats_1d, lons, lats, mslp, rain, tmp, uwnd, vwnd, title, output_filename):
    #
    # MapRegion Classの初期化
    ...
    levels2 = range(math.floor(mslp.min()-math.fmod(mslp.min(),2)), math.ceil(mslp.max())+1,2)
    # 等圧線をひく
    cr2 = m.contour(lons, lats, mslp, levels=levels2, colors='k', linewidths=0.8)
    ...
    if opt_stmp:
        ...
        levelst = [2]
        # 等温線をひく
        cr3 = m.contour(lons, lats, tmp, levels=levelst, colors='cornflowerblue', linestyles='-', linewidths=0.8)
        ...
        levelst = [-2]
        # 等温線をひく
        cr4 = m.contour(lons, lats, tmp, levels=levelst, colors='blue', linestyles='-', linewidths=0.8)
...
if __name__ == '__main__':
    ...
    for fcst_time in np.arange(fcst_str, fcst_end+1, fcst_step):
        ...
        plotmap(fcst_time, sta, lons_1d, lats_1d, lons, lats, mslp, rain, tmp, uwnd, vwnd, \
            title, output_filename)
        output_filenames.append(output_filename)
    # gifアニメーションのファイル名
    output_gif_filename = "anim_msm_mslp_" + sta + ".gif"
    # pngからgifアニメーションに変換
